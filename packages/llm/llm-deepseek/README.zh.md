...
该插件还会在可配置提供方目录（`ctx.llm.listConfigurableProviders()`）中声明自己的路由：提供方为 `deepseek-official`，settings namespace 为 `llm-deepseek`，settings path 为空——整个分节就是 profile。只要插件挂载，路由就会注册，因此已存储或环境中的密钥无需额外设置即可服务请求；在 Models 页面上，只有当其分节携带用户层或非默认组合内容时，该提供方才会显示为一行，在此之前它与其他休眠提供方一样通过该页面的添加流程提供。
