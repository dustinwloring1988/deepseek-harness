 ...
 * solid configured or missing dots. Providers the user has not added yet —
 * including a dormant DeepSeek — sit in the add card's select instead of
 * rendering rows. Each card kind owns its own open state, so closing one
 * never discards a draft in another. Every mutation writes through the wire,
 * while a provider removal first requires confirmation; the page re-renders
 * from pushed invalidations or the post-apply reload.
 */
