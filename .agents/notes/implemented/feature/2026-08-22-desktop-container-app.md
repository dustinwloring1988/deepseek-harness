...
The shipped launch path is the CLI's `desktop` subcommand (`apps/cli/src/desktop-launch.ts`): it verifies the sibling container-app checkout is built, resolves the Electron binary through that app's dependency graph, spawns it with the app directory plus verbatim forwarded arguments, and adopts the child's exit code; a missing checkout, build, or install fails loud naming the fix.
...
- **A native IPC bridge instead of any HTTP server** (load the frontend over `file://` and carry `ctx.apiProxy` across Electron IPC via the client transport hooks) — deferred: it reworks `dsh-client-connection`'s trust fences and the boot kernel seams for no current need, and the loopback transport is already same-origin fenced. The seam is documented as available if a no-socket deployment ever demands it.
...
- **A `dsh desktop` hardcoded profile alias like `dsh web`** — rejected: desktop is not a Cordis composition to boot; the Electron process must own the backend spawn so its single-instance lock, window lifecycle, and tree-kill stay in one place. The subcommand is a thin resolver-and-spawn instead.
- **Resolving Electron through a `dsh` → `@deepseek-ai/dsh-desktop` dependency** — rejected: the dependency direction is already inverted (the container app depends on `@deepseek-ai/dsh` to spawn its built CLI), so a reverse edge would cycle; the launcher anchors module resolution at the sibling checkout instead.
