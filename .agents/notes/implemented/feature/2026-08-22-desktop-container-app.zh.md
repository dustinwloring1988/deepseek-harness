...
此前使用该 agent 只有两条路径：在终端手动启动 `dsh web` 服务再开浏览器标签页，或使用 headless 一次性 CLI。想要"这个应用"的用户必须打开终端、启动服务、记下端口、再把浏览器指向它；关闭浏览器后服务仍在运行，也没有任何一个窗口完整承载产品。

## 决策

`apps/desktop`（`@deepseek-ai/dsh-desktop`）是一个 Electron 容器，其主进程拥有完整技术栈：挑选一个空闲的 127.0.0.1 端口，以 `--profile web --port <端口> --no-open` 启动 `apps/cli` 构建出的 `lib/bin.js`，等待该端口响应 HTTP，再在单一窗口中加载它。关闭窗口会终止后端整个进程树（Windows 上 `taskkill /T`，其余平台 `SIGTERM`）。通过单实例锁，再次启动只会聚焦已有窗口，而不会针对另一个数据目录启动第二个后端。跨源导航会被拒绝，新窗口请求转交默认浏览器。
...
交付的启动路径是 CLI 的 `desktop` 子命令（`apps/cli/src/desktop-launch.ts`）：校验同级容器应用检出目录已完成构建，通过该应用的依赖图解析 Electron 二进制，以应用目录加上原样转发的参数启动它并采用子进程退出码；检出缺失、未构建或未安装都会失败并指明修复方式。

## 备选方案

- **Tauri** —— 否决：它会为纯 Node 仓库引入 Rust 工具链与 sidecar 进程打包，而后端运行时本就需要 Node；Electron 保持单一语言和单一进程模型。
- **用原生 IPC 取代一切 HTTP 服务**（前端走 `file://` 加载，`ctx.apiProxy` 经 Electron IPC 由 client transport hooks 承载）—— 延后：它需要改造 `dsh-client-connection` 的信任围栏与 boot kernel 各接缝，而当前并无此需求，且回环传输已具备同源围栏。若未来出现禁用套接字的部署形态，该接缝已被文档化为可用。
...
- **像 `dsh web` 那样做 `dsh desktop` 硬编码 profile 别名** —— 否决：desktop 不是要启动的 Cordis 组合；Electron 进程必须拥有后端派生，其单实例锁、窗口生命周期与进程树终止才能留在同一处。该子命令只做薄薄的解析与派生。
- **通过 `dsh` → `@deepseek-ai/dsh-desktop` 依赖解析 Electron** —— 否决：依赖方向已经相反（容器应用依赖 `@deepseek-ai/dsh` 来派生其构建后的 CLI），反向边会成环；启动器改为锚定同级检出目录做模块解析。
