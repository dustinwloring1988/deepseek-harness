// Keyless browser e2e: a fresh user is never prompted for the official
// DeepSeek key. The route ships dormant, the Models page offers it through the
// add card like any other provider, and adding it there stores the profile and
// the typed key in an isolated harness home without a reload or model call.
...
describe.skipIf(MODE === 'record')('web e2e: adding the dormant official DeepSeek route', () => {
  ...
  it('offers DeepSeek unadded, stores the first key through the add card, and never prompts', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-deepseek-add'))
    ...
    // No credential step exists anymore: dismissing the notice leaves an
    // ordinary app, and nothing takes over while no provider is configured.
    ...
    // Dormant: DeepSeek owns no row until the user adds it.
    ...
    // The add landed as a deletable row with the configured-key dot.
    const deepSeekRow = settings.getByText('DeepSeek', { exact: true }).first()
    await deepSeekRow.waitFor({ timeout: 10_000 })
    await deepSeekRow.locator('xpath=ancestor::li').getByRole('img', { name: 'API 密钥已配置' })
      .waitFor({ timeout: 10_000 })
    await deepSeekRow.locator('xpath=ancestor::li').getByRole('button', { name: '删除 DeepSeek (deepseek-official)' })
      .waitFor({ timeout: 10_000 })

    // The profile records the derived reference; the key value lands in the
    // harness home's .credentials.yaml and nowhere in the page.
    await expect.poll(
      async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'),
      { timeout: 10_000 },
    ).toContain('apiKeyEnv: DEEPSEEK_OFFICIAL_API_KEY')
    const stored = await readFile(join(scaffold.harnessHome, '.credentials.yaml'), 'utf8')
    expect(stored.includes(`DEEPSEEK_OFFICIAL_API_KEY: ${secret}`)).toBe(true)
    ...
    // and still nothing else takes over.
  ...
  it('never paints takeover chrome while the shell decides, even with the settings join held open', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-configured-reload'))
    // Regression pin for the reload flash: the remaining step must load its
    // private facts before deciding not to show. Dialog chrome lives inside
    // each visible branch, so the deciding window paints and blocks nothing.
    // Holding settings.describe widens that window from loopback RTT scale to
    // a deterministic hundreds of milliseconds, removing all timing dependence
    // from the sampler assertions below.
    //
    ...
    await page.addInitScript(() => {
      ...
      setInterval(() => {
        if (document.querySelector('[role="dialog"][aria-label="内测声明"]') !== null) {
  ...
  it('configures arbitrary DeepSeek models and reflects them in the composer menu', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-deepseek-models'))
    // Opened here rather than inherited: the add test reloads the page after
    // storing the key, so nothing carries an open dialog across.
    await page.getByRole('button', { name: '设置', exact: true }).click()
    const settings = page.getByRole('dialog', { name: '设置' })
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: '模型' }).click()
