...
The desktop container app: one Electron window around the harness Web GUI. Its main process picks a free loopback port, spawns the built `dsh web` backend on it, waits until that port answers HTTP, and loads it in the window; closing the window stops the whole backend process tree.
