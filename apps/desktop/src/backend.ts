/**
 * The spawned `dsh web` backend process: pick a free loopback port, launch the
...
async function pollUntilAnswering(url: string, deadlineMs: number): Promise<void> {
  ...
  for (;;) {
    ...
    if (Date.now() > deadline) throw new Error(`dsh web did not answer ${url} within ${String(deadlineMs)}ms`)
    await new Promise((resolve) => { setTimeout(resolve, BACKEND_POLL_INTERVAL_MS) })
  }
}

/**
 * Spawn the built dsh CLI's web profile on a freshly picked port and wait
...
export async function spawnWebBackend(cliBinPath: string, options: SpawnWebBackendOptions = {}): Promise<WebBackend> {
  ...
  const ready = (async () => {
    const exited = new Promise<never>((_, reject) => {
      exitListener = (code) => { reject(new Error(`dsh web exited before answering requests (code ${String(code)})`)) }
