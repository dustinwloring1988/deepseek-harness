...
This reference defines the profile, web-alias, desktop, plugin-management, and config-dump command modes. Argv is parsed once through [`src/args.ts`](../src/args.ts), and [`src/bin.ts`](../src/bin.ts) dynamically imports only the selected runner.
...
`dsh desktop` launches the Electron container app from [`apps/desktop`](../../../apps/desktop/README.md) instead of booting a profile: it verifies the sibling checkout is built, resolves the Electron binary through that app's dependency graph, and spawns it with the app directory plus every remaining argument forwarded verbatim (Electron and Chromium switches pass through). Electron owns no useful `-h`, so unlike an app alias, `-h`/`--help` prints this command's own usage and exits 0. The container app then spawns the built `dsh web` backend on a loopback port itself, so this mode needs built artifacts (`pnpm run build`) but no profile directory. A missing checkout, missing build, or missing `pnpm install` fails with a diagnostic naming the fix; the child's exit code becomes this invocation's exit code.
